...
data class FourFoldKent(@CsvBindByPosition(position = 0) val dan: BigDecimal = BigDecimal.ZERO,
  ...
  override fun add(ff: FourFold): FourFold {
    if(ff is FourFoldKent){
      return FourFoldKent(dan.plus(ff.dan) , mat.plus(ff.mat), paulS.plus(ff.paulS) , paulV.plus(ff.paulV), frank.plus(ff.frank), jase.plus(ff.jase), ash.plus(ff.ash))
    }
    return FourFoldKent()
  }
  ...
  override fun roundWinnings(): List<Pair<String, BigDecimal>> {
    return listOf(Pair("mat", mat), Pair("dan", dan), Pair("paulS", paulS), Pair("paulV", paulV), Pair("frank", frank), Pair("jase", jase), Pair("ash", ash)).sortedBy { n -> n.second }.asReversed()
  }
